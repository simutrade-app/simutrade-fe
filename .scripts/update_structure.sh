...
echo "# simutrade-frontend Structure" > .cursor/rules/structure.mdc
echo "" >> .cursor/rules/structure.mdc
echo '```' >> .cursor/rules/structure.mdc

# Change to simutrade-frontend directory
(cd simutrade-frontend && {
  # Check if tree command is available
  if command -v tree &> /dev/null; then
    # Use tree command for better visualization directly
    tree -a -I ".git|node_modules" . >> ../.cursor/rules/structure.mdc
    ...
    git ls-files --others --exclude-standard --cached . | sort > /tmp/sfe_files_list.txt

    # Create a simple tree structure
    echo "." > /tmp/sfe_tree_items.txt
    ...
    while read -r file; do
      ...
      echo "$file" >> /tmp/sfe_tree_items.txt

      # Add all parent directories
      dir="$file"
      while [[ "$dir" != "." ]]; do
        dir=$(dirname "$dir")
        echo "$dir" >> /tmp/sfe_tree_items.txt
      done
    done < /tmp/sfe_files_list.txt

    # Sort and remove duplicates
    sort -u /tmp/sfe_tree_items.txt > /tmp/sfe_tree_sorted.txt
    mv /tmp/sfe_tree_sorted.txt /tmp/sfe_tree_items.txt
    ...
    while read -r item; do
      ...
      for ((i=0; i<depth; i++)); do
        if [[ $i -lt $common ]]; then
          ...
          has_more=0
          for next in $(grep "^$(dirname "$item")/" /tmp/sfe_tree_items.txt); do
            if [[ "$next" > "$item" ]]; then
              has_more=1
              break
            fi
      ...
      is_last=1
      dir=$(dirname "$item")
      for next in $(grep "^$dir/" /tmp/sfe_tree_items.txt); do
        if [[ "$next" > "$item" ]]; then
          is_last=0
          break
        fi
      ...
      # Output the item - prefix with simutrade-frontend
      if [[ "$item" == "." ]]; then
        echo "simutrade-frontend" >> ../.cursor/rules/structure.mdc
      else
    ...
    done < /tmp/sfe_tree_items.txt

    # Clean up
    rm -f /tmp/sfe_files_list.txt /tmp/sfe_tree_items.txt
  fi
})
